...
class qResults(qResBase):
    ...
    def readFrom(self, file, readKey):
        oldDict = self._qResBase__results
        rDict = self._readFrom(file, keyTo=readKey, keyCo=None, oldDict=oldDict)
        self._qResBase__results = rDict
        for k, v in oldDict.items():
            if v not in [[], {}]:
                self._qResBase__results[k] = v
        self._qResBase__resultsLast = self._qResBase__results

    def _readFrom(self, file, keyTo=None, keyCo=None, oldDict=None):
        rDict = {}
        if not self._keySearch(oldDict, keyTo):
            if hasattr(file, 'items'):
                for key, val in file.items():
                    if keyTo == key:
                        print(keyTo, keyCo)
                        rDict[key] = self._readFrom(val, keyTo=None, keyCo=key, oldDict=oldDict)
                    else:
                        rDict[key] = self._readFrom(val, keyTo=keyTo, keyCo=key, oldDict=oldDict)
            else:
