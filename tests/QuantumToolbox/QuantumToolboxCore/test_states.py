...
def test_basisByRandom(helpers):
    ...
    for _ in range(5):
        ...
        st = states.basis(dim, exc).toarray()
        assert st[exc] == 1
...
def test_superPosIntInput(helpers):
    ...
    for _ in range(5):
        dim, exc = helpers.generateRndDimAndExc(0)
        assert np.allclose(states.superPos(dim, exc).toarray(), states.basis(dim, exc).toarray())
...
def test_superPosListInput(helpers):
    ...
    assert np.allclose(states.superPos(2, [0, 1]).toarray(), np.array([[0.70710678], [0.70710678]]))
    assert np.allclose(states.superPos(3, [0, 1, 2]).toarray(), (1/np.sqrt(3))*(states.basis(3, 0) + states.basis(3, 1) +
                                                                        states.basis(3, 2)).toarray())
    for _ in range(5):
        dim, comps = helpers.generateRndStateParams()
        state = sum([np.sqrt(1/len(comps))*states.basis(dim, k) for k in comps])
        assert np.allclose(states.superPos(dim, comps.keys()).toarray(), state.toarray())
...
def test_superPosDictInput(helpers):
    ...
    for _ in range(5):
        ...
        assert np.allclose(states.superPos(dim, excs).toarray(), state.toarray())
        state, dim, excs = helpers.generateRndPureState(2)
        state = (1/la.norm(state))*state
        assert np.allclose(states.superPos(dim, excs, populations=False).toarray(), state.toarray())
...
def test_densityMatrixRandomPure(helpers):
    ...
    for _ in range(5):
        ...
        assert np.allclose(states.densityMatrix(state).toarray(), la.outerProd(state).toarray())
...
def test_densityMatrixRandomMixed(helpers):
    ...
    for _ in range(5):
        ...
        assert np.allclose(states.densityMatrix(comps, excs.values()).toarray(), state.toarray())
...
def test_BellStates(name, specialQubitStates): #pylint:disable=invalid-name
    ...
    assert np.allclose(states.BellStates(name).toarray(), specialQubitStates['Bell'+name])
