...
def test_QTermCreateTerm():
    ...
    t1 = QTerm()
    t1._createTerm(named1, sigmaz)
    assertTheseVals(t1, named1, sigmaz, 1, None, 0)

    t1._createTerm(named1, sigmaz, orders=2, frequency=3)
    assertTheseVals(t1, named1, sigmaz, 2, 3, 0)

    t1 = QTerm()
    t1._createTerm([named1, named2, named3], [sigmaz, sigmay, sigmax])
    assertTheseVals(t1, [named1, named2, named3], [sigmaz, sigmay, sigmax], [1, 1, 1], None, 3)

    t1 = QTerm()
    t1._createTerm([named1, named2, named3, named4], [sigmaz,sigmaz, sigmay, sigmax], orders=[0, 1, 2, 3], frequency=4)
