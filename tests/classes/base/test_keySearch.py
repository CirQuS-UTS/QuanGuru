...
def test_keySearchWithRegularStrings():
    ...
    someString = randString(random.randint(1, 10))
    assert qbase.keySearch(dictionary, someString) == someString
    
def test_keySearchWithAliasClassAndRegularStrings(): # pylint: disable=too-many-statements
    # create a dictionary first
    dictionary = {key : random.randint(1, 10) for key in strings}
    ...
    aliasObj3 = qbase.aliasClass(name=randString(random.randint(1, 10)),
                                alias=[aliasObj1.alias[0], randString(random.randint(1, 10))])
