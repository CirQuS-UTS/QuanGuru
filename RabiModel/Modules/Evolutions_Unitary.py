...
def idealRabi(obj, stepSize):
    HamRabi = 2 * np.pi * (obj.resonatorFrequency * obj.HamiltonianCavity + obj.qubitFreq * obj.HamiltonianQubit
...
def digitalRabi(obj, stepSize):
    HamJC = 2 * np.pi * (((obj.offset + obj.resonatorFrequency/obj.ratio) * obj.HamiltonianCavity) +
                         ((obj.offset + obj.qubitFreqJC) * obj.HamiltonianQubit) + (obj.g * obj.couplingJC))
    UnitaryJC = liou.Liouvillian(HamJC, timeStep=(stepSize / 2))

    if obj.qubitFreqAJC == obj.qubitFreqJC:
        UnitaryAJC = (UnitaryJC @ UnitaryJC)
    else:
        HamAJC = 2 * np.pi * ((obj.offset + obj.resonatorFrequency / obj.ratio) * obj.HamiltonianCavity
                              + (obj.offset + obj.qubitFreqAJC) * obj.HamiltonianQubit + obj.g * obj.couplingJC)
        UnitaryAJC = liou.Liouvillian(HamAJC, timeStep=stepSize)

    if obj.offset == 0:
        Unitary = UnitaryJC @ obj.sigmaX @ UnitaryAJC @ obj.sigmaX @ UnitaryJC
    else:
        half_flip = 2 * np.pi * ((obj.offset + obj.resonatorFrequency / obj.ratio) * obj.HamiltonianCavity +
