...
def digitalRabi(obj, stepSize):
    HamJC = 2 * np.pi * (((obj.subSystems[0].frequency/6)*obj.subSystems[0].freeHam) + obj.couplingHam)
...
cavParity = qOps.parityEXP(cav1.Matrix)
...
surf1 = ax.pcolormesh(X, Y, parityIdeal)



fig2, ax2 = plt.subplots()
Y, X = np.meshgrid(qSim.times, qSim.sweepList)
surf2 = ax2.pcolormesh(X, Y, parityDigit)
plt.show()